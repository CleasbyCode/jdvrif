...
[[nodiscard]] fs::path writeToStagedOutput(WriteFn&& write_fn) {
    static thread_local std::array<char, OUTPUT_STREAM_BUFFER> tls_stream_buf;
    ...
    // pubsetbuf must be called before any I/O; openBinaryOutputForWriteOrThrow
    // only opens the file, so this is the right window. Cuts per-MB write()
    // syscall count from ~1024 (default 4 KiB streambuf) to 1.
    file_ofs.rdbuf()->pubsetbuf(tls_stream_buf.data(), tls_stream_buf.size());
    write_fn(file_ofs);
...
[[nodiscard]] ConcealFinalizeResult concealBlueskyPath(
    ...
    // encryptDataFileForBluesky no longer pre-concatenates segment_vec into the
    // cover bytes; saveEmbeddedJpg writes them as two consecutive spans below.
    vBytes jpg_holder;  // intentionally empty; the cover is supplied as a span.
