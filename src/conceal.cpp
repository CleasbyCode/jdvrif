...
[[nodiscard]] EncryptionInput prepareEncryptionInput(const fs::path& data_file_path,
    ...
    const fs::path& compressed_path = compressed_stage.emplace(fs::path{}, "comp").path();
    zlibCompressFileToPath(data_file_path, compressed_path, source_data_size);

    return EncryptionInput{
        .path = compressed_path,
        .size = checkedFileSize(compressed_path, "Zlib Compression Error: Failed to build compressed payload.", true),
        .is_compressed = true,
    };
}
...
void validateCombinedSizeLimits(std::size_t encrypted_payload_size, std::size_t jpg_size, const ConcealFlags& flags) {
    ...
    // segments hold ~171 KB, well under MAX_EMBEDDED_CIPHERTEXT_BLUESKY, and
...
[[nodiscard]] ConcealFinalizeResult concealRedditPath(
    ...
    // Reddit always compresses, including file types that the default conceal
    // path would bypass above 10 MiB.
    StagingFile compressed_stage({}, "reddit_comp");
    zlibCompressFileToPath(
        data_file_path,
        compressed_stage.path(),
        source_data_size);
    const std::size_t compressed_size = checkedFileSize(
        compressed_stage.path(),
        "Zlib Compression Error: Failed to build Reddit compressed payload.",
        true);

    if (compressed_size > cover.payload_capacity) {
        std::println(
        ...
        throw std::runtime_error(std::format(
            ...
            compressed_size,
            cover.payload_capacity,
            cover.payload_capacity / 1000));
    }

    throwIf(
        data_filename.size() >
            std::numeric_limits<std::size_t>::max() - 1 - compressed_size,
    ...
    if (envelope_size > cover.payload_capacity) {
        throw std::runtime_error(std::format(
            "Data File Size Error: After encryption and required recovery metadata, "
            "the Reddit payload is {} bytes and exceeds the theoretical maximum "
            "limit of {} bytes for this cover image.",
            envelope_size,
            cover.payload_capacity));
    }
    ...
    vBytes embedded_jpeg = embedRedditPayload(cover, envelope);
...
void concealData(vBytes& jpg_vec, Option option, const fs::path& data_file_path) {
    ...
    // Bypass needs a >10 MB source, which can never fit the ~171 KB Bluesky
