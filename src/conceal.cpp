...
[[nodiscard]] fs::path saveEmbeddedJpg(std::span<const Byte> segment_vec, std::span<const Byte> jpg_vec) {
    ...
    try {
        std::ofstream file_ofs(temp_output_path, std::ios::binary);
        if (!file_ofs) {
            throw std::runtime_error(
        ...
        if (ec) {
            throw std::runtime_error(std::format("Write File Error: Failed to commit output image: {}", ec.message()));
        }
    } catch (...) {
...
void finalizeDefaultPlatformReport(vString& platforms_vec, vBytes& segment_vec, vBytes& jpg_vec, std::size_t embedded_jpg_size) {
    const std::span<const Byte> vec = segment_vec.empty()
        ? std::span<const Byte>(jpg_vec)
        : std::span<const Byte>(segment_vec);

    if (vec.size() <= TOTAL_SEGMENTS_INDEX + 1 || vec.size() <= FIRST_SEGMENT_SIZE_INDEX + 1) {
        throw std::runtime_error("File Extraction Error: Corrupt segment metadata.");
    }
...
void concealData(vBytes& jpg_vec, Option option, const fs::path& data_file_path) {
    ...
    if (flags.has_no_option) {
        finalizeDefaultPlatformReport(platforms_vec, segment_vec, jpg_vec, embedded_jpg_size);
    }
