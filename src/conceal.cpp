...
void concealData(vBytes& jpg_vec, Mode mode, Option option, const fs::path& data_file_path) {
    ...
    // These could be loaded from a binary resource to improve readability.
    vBytes segment_vec {
    ...
    if (hasNoOption) {
        ...
        const auto first_segment_size = static_cast<uint16_t>(getValue(vec, FIRST_SEGMENT_SIZE_INDEX));
        const auto total_segments     = static_cast<uint16_t>(getValue(vec, TOTAL_SEGMENTS_INDEX));
