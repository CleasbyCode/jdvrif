...
void validateCoverImageLimits(std::size_t jpg_size, const ConcealFlags& flags) {
    if (jpg_size > MAX_OPTIMIZED_IMAGE_SIZE) {
        throw std::runtime_error("Image File Error: Cover image exceeds default maximum size limit for all conceal modes.");
    }
    if (flags.has_bluesky_option && jpg_size > MAX_BLUESKY_IMAGE_SIZE) {
        throw std::runtime_error("File Size Error: Cover image exceeds the maximum size limit of 2,000,000 bytes for the Bluesky platform. Use a smaller cover image.");
    }
}
...
void flushStdoutOrThrow() {
    if (std::fflush(stdout) != 0 || std::ferror(stdout) != 0) {
        throw std::runtime_error(PIN_DELIVERY_ERROR);
    }
}
...
void concealData(vBytes& jpg_vec, Option option, const fs::path& data_file_path) {
    ...
    if (flags.has_bluesky_option && bypass_compression) {
        throw std::runtime_error(
