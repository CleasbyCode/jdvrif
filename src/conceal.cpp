...
[[nodiscard]] vBytes makeSegmentTemplate(bool has_bluesky_option) {
    if (has_bluesky_option) {
        return vBytes(BLUESKY_EXIF_SEGMENT.begin(), BLUESKY_EXIF_SEGMENT.end());
    }
    return vBytes(DEFAULT_ICC_SEGMENT.begin(), DEFAULT_ICC_SEGMENT.end());
}
...
void concealData(vBytes& jpg_vec, Option option, const fs::path& data_file_path) {
    ...
    ConcealFinalizeResult result = flags.has_bluesky_option
        ? concealBlueskyPath(segment_vec, jpg_vec, encryption_input, data_filename, platforms_vec)
        : concealDefaultPath(segment_vec, jpg_vec, encryption_input, data_filename, flags.has_reddit_option, platforms_vec);
