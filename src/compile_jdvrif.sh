...
# -march=native tunes and emits instructions for THIS machine's CPU (for example
# AVX2). The resulting binary is NOT redistributable: running it on an older or
# different CPU can fail with "Illegal instruction" (SIGILL). jdvrif is meant to
# be built from source on the machine that runs it. If you must ship a portable
# binary, replace this with a baseline arch (for example -march=x86-64-v2) or
# use -mtune=native for portable codegen; the base64 SIMD tier in base64.cpp is
# selected at compile time from these flags, so a baseline arch without AVX2
...
echo "Round-trip tests: bash tests/run_roundtrip_tests.sh"
