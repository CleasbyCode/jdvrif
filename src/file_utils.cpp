...
[[nodiscard]] std::size_t checkedPathSize(const fs::path& path, std::string_view error_message, bool require_non_empty = false, bool require_stream_sized = false) {
    ...
    if (require_non_empty && file_size == 0) {
        throw std::runtime_error(std::string(error_message));
    }
    return file_size;
}
...
std::ofstream openBinaryOutputForWriteOrThrow(const fs::path& path) {
    ...
    if (!output) {
        throw std::runtime_error("Write Error: Unable to create output file. Make sure you have WRITE permissions for this location.");
    }
    restrictOutputPermissionsOrThrow(path, "Write Error: Failed to secure output file permissions");
    return output;
}

OutputFile::OutputFile(const fs::path& path, std::size_t buffer_capacity)
    : buffer_(buffer_capacity) {
    // O_EXCL + mode 0600 == std::ios::noreplace + restrictOutputPermissions:
    // atomic create that fails if the path exists, owner read/write only.
...
void OutputFile::writeRaw(const Byte* data, std::size_t size, std::string_view error_message) {
    while (size > 0) {
        const ssize_t got = ::write(fd_, data, size);
        if (got < 0) {
...
void OutputFile::drain(std::string_view error_message) {
    if (fill_ == 0) return;
    writeRaw(buffer_.data(), fill_, error_message);
    fill_ = 0;
}
...
void OutputFile::write(std::span<const Byte> bytes, std::string_view error_message) {
    ...
    if (bytes.size() >= buffer_.size()) {
        drain(error_message);
        writeRaw(bytes.data(), bytes.size(), error_message);
        return;
    }
...
void OutputFile::sendFromFallback(int in_fd, std::size_t in_offset, std::size_t length, std::string_view error_message) {
    // drain() has already run, so buffer_ is free scratch space.
    off_t off = static_cast<off_t>(in_offset);
    std::size_t left = length;
    while (left > 0) {
        const std::size_t want = std::min(left, buffer_.size());
        std::size_t got_total = 0;
        while (got_total < want) {
            const ssize_t got = ::pread(in_fd, buffer_.data() + got_total, want - got_total, off + static_cast<off_t>(got_total));
            if (got < 0) {
                if (errno == EINTR) continue;
                throw std::runtime_error(std::string(error_message));
            }
            if (got == 0) throw std::runtime_error(std::string(error_message));  // unexpected EOF
            got_total += static_cast<std::size_t>(got);
        }
        writeRaw(buffer_.data(), want, error_message);
...
void OutputFile::sendFrom(int in_fd, std::size_t in_offset, std::size_t length, std::string_view error_message) {
    ...
    while (left > 0) {
        ...
        const ssize_t n = ::sendfile(fd_, in_fd, &offset, want);
        if (n < 0) {
            if (errno == EINTR) continue;
            if ((errno == EINVAL || errno == ENOSYS) && left == length) {
                ...
                sendFromFallback(in_fd, in_offset, length, error_message);
                return;
            }
            throw std::runtime_error(std::string(error_message));
        }
        if (n == 0) throw std::runtime_error(std::string(error_message));  // unexpected EOF
        left -= static_cast<std::size_t>(n);
    }
}

void OutputFile::close(std::string_view error_message) {
    if (fd_ < 0) return;
    drain(error_message);
    const int rc = ::close(fd_);
    fd_ = -1;
    if (rc != 0) throw std::runtime_error(std::string(error_message));
}
...
void validateFilenameProperties(const fs::path& path, FileTypeCheck file_type) {
    ...
    if (isImageType(file_type) && !hasFileExtension(path, {".jpg", ".jpeg", ".jfif"})) {
