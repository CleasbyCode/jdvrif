...
[[nodiscard]] bool isReservedEmbeddedFilename(std::string_view filename) {
    return filename == "." || filename == ".." || (!filename.empty() && (filename.front() == '.' || filename.front() == '-'));
}
...
std::ofstream openBinaryOutputTruncOrThrow(const fs::path& path, std::string_view error_message) {
    ...
    if (!output) throw std::runtime_error(std::string(error_message));
...
std::size_t validateFileForRead(const fs::path& path, FileTypeCheck FileType) {
    ...
    if (isImageType(FileType)) {
        if (!hasFileExtension(path, {".jpg", ".jpeg", ".jfif"})) throw std::runtime_error("File Type Error: Invalid image extension. Only expecting \".jpg\", \".jpeg\", \".jfif\".");
