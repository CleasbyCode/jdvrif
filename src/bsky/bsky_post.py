...
def _safe_download(url: str, max_bytes: int, timeout: int = 10) -> bytes:
    ...
    for redirects_followed in range(MAX_REDIRECTS + 1):
        ...
        try:
            ...
            return _read_response_body(resp, max_bytes)
        finally:
            resp.close()
...
def fetch_embed_url_card(pds_url: str, access_token: str, url: str) -> Dict:
    ...
    _attach_external_thumb(card, pds_url, access_token, url, soup)
...
def test_span_reservations_stay_sorted():
    occupied: List[tuple[int, int]] = []
    assert _reserve_span({"start": 10, "end": 20}, occupied)
    assert _reserve_span({"start": 0, "end": 5}, occupied)
    assert not _reserve_span({"start": 4, "end": 12}, occupied)
    assert _reserve_span({"start": 20, "end": 25}, occupied)
    assert occupied == [(0, 5), (10, 20), (20, 25)]
...
def test_read_response_body_limits():
    ...
    assert _read_response_body(FakeResponse([b"ab", b"", b"cd"]), 4) == b"abcd"
    try:
...
def test_get_reply_refs_reuses_root_ref():
    ...
    assert calls == ["at://did:plc:parent/app.bsky.feed.post/parent"]
    assert refs == {
        "root": {
            "uri": "at://did:plc:root/app.bsky.feed.post/root",
            "cid": "root-cid",
