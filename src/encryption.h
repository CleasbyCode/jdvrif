...
[[nodiscard]] std::uint64_t encryptDataFileFromPathToFile(
    vBytes& segment_vec,
...
[[nodiscard]] std::uint64_t encryptDataFile(vBytes& segment_vec, vBytes& data_vec, vBytes& jpg_vec,
    vString& platforms_vec, const std::string& data_filename, bool hasBlueskyOption, bool hasRedditOption);

struct DecryptRequest {
    const fs::path* stream_output_path = nullptr;
...
[[nodiscard]] DecryptResult decryptDataFile(vBytes& jpg_vec, bool isBlueskyFile,
