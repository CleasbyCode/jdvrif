...
// Reddit cannot preserve APP/ICC metadata, so return the 56-byte KDF block
// separately.  The caller places it in the DCT-carried Reddit envelope next to
// the ciphertext.
...
// Reddit carries only the compact KDF block rather than an APP/EXIF metadata
// template. Validates it, prompts for the PIN, and derives the same existing
// secretstream key/header pair.
