...
[[nodiscard]] bool pullSecretStreamFrame(
    ...
    if (cipher_frame.size() < STREAM_ABYTES || cipher_frame.size() > plain_capacity + STREAM_ABYTES) {
        return false;
    }
...
[[nodiscard]] bool decryptWithSecretStreamChunks(
    ...
    ZeroGuard<PlainChunk> guard{plain_chunk.get()};
...
[[nodiscard]] bool decryptWithSecretStream(vBytes& data_vec, const Key& key, const StreamHeader& header) {
    // Plaintext is strictly smaller than ciphertext (each frame loses 4 + ABYTES bytes),
    // so the plaintext write index always trails the ciphertext read offset. Decrypt in place.
    ...
    const bool ok = decryptWithSecretStreamChunks(
        ...
        [&](std::span<const Byte> chunk) {
            std::memcpy(data_vec.data() + write_index, chunk.data(), chunk.size());
            write_index += chunk.size();
        });
    if (!ok) {
        return false;
    }

    if (write_index < data_vec.size()) {
        sodium_memzero(data_vec.data() + write_index, data_vec.size() - write_index);
    }
    data_vec.resize(write_index);
    return true;
}
