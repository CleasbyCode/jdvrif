...
// alone leaves backspaced PIN digits in the heap freelist.
inline void wipeStringCapacity(std::string& input) noexcept {
    if (const std::size_t cap = input.capacity(); cap > 0) {
        try {
            input.resize(cap, '\0');
        } catch (...) {
            if (!input.empty()) {
...
struct WipeStringGuard {
    std::string* str{nullptr};

    explicit WipeStringGuard(std::string& s) noexcept : str(&s) {}
    WipeStringGuard(const WipeStringGuard&) = delete;
    WipeStringGuard& operator=(const WipeStringGuard&) = delete;

    ~WipeStringGuard() {
        if (str) wipeStringCapacity(*str);
    }
};
...
inline void requirePlatformEntries(const vString& platforms_vec) {
    if (platforms_vec.size() <= BLUESKY_PLATFORM_INDEX) {
