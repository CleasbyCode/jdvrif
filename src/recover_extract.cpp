...
[[nodiscard]] std::size_t extractBlueskyCiphertextToFile(const fs::path& image_path, std::size_t image_size, std::size_t embedded_file_size, const fs::path& output_path) {
    ...
    const auto pshop_sig_opt = findSignatureInFile(image_path, BLUESKY_PHOTOSHOP_SIGNATURE);
    ...
    if (pshop_segment_size > image_size - pshop_segment_size_index) throw std::runtime_error(CORRUPT_FILE_ERROR);
    ...
    const auto xmp_sig_opt = findSignatureInFile(image_path, BLUESKY_XMP_CREATOR_SIGNATURE, pshop_segment_size_index, xmp_search_start);
    if (!xmp_sig_opt) throw std::runtime_error(CORRUPT_FILE_ERROR);

    const std::size_t base64_begin_index = *xmp_sig_opt + BLUESKY_XMP_CREATOR_SIGNATURE.size() + 1;
