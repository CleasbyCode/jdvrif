...
[[nodiscard]] bool hasSignatureAt(std::ifstream& input, std::size_t image_size, std::size_t offset, std::span<const Byte> signature) {
    ...
    if (signature.empty() ||
        signature.size() > MAX_SIG_BYTES ||
        offset > image_size ||
        signature.size() > image_size - offset) {
        return false;
    }
...
// ---------------------------------------------------------------------------
// POSIX fd wrappers. Used by the extract paths so they can sendfile(2) bytes
// straight from the input image to the staging cipher file without dragging
// the payload through user space. Sig scans and the small reads at known
// offsets (readU16At, hasSignatureAt) continue to use std::ifstream — they
// are not bandwidth-bound and reuse infrastructure that recover.cpp needs.
// ---------------------------------------------------------------------------
...
class FdOutputFile {
public:
    FdOutputFile(const fs::path& path, std::string_view error_message) {
        // O_EXCL is the POSIX equivalent of std::ios::noreplace; combined with
        // mode 0600 this matches openBinaryOutputForWriteOrThrow's behaviour
        // (atomic create + owner-only perms) without a separate chmod call.
    ...
    ~FdOutputFile() noexcept {
        if (fd_ >= 0) ::close(fd_);
    }

    FdOutputFile(const FdOutputFile&) = delete;
    FdOutputFile& operator=(const FdOutputFile&) = delete;

    [[nodiscard]] int fd() const noexcept { return fd_; }

    void writeAll(std::span<const Byte> bytes, std::string_view error_message) {
        const Byte* p   = bytes.data();
        std::size_t rem = bytes.size();
        while (rem > 0) {
            const ssize_t got = ::write(fd_, p, rem);
            if (got < 0) {
...
// Kernel-to-kernel copy via sendfile(2). Source fd advances internally via the
// off_t* argument; dest fd uses its current file position.
void sendfileExact(int in_fd, int out_fd, std::size_t in_offset, std::size_t length, std::string_view error_message) {
    off_t offset = static_cast<off_t>(in_offset);
    while (length > 0) {
        // sendfile() on Linux is capped per-call at 0x7ffff000 (just under 2 GB)
        // for historical reasons. Clamp so a single call always succeeds.
        constexpr std::size_t SENDFILE_MAX = 0x7ffff000;
        const std::size_t want = std::min(length, SENDFILE_MAX);
        ...
        if (n == 0) {
            throw std::runtime_error(std::string(error_message));   // premature EOF
        }
...
// Copy a contiguous run [offset, offset+length) from the input image straight
// into the staging file via sendfile(2). The bytes never touch user space.
void copyRangeFdToFd(int in_fd, int out_fd, std::size_t offset, std::size_t length) {
    sendfileExact(in_fd, out_fd, offset, length,
...
[[nodiscard]] std::size_t streamDecodeBase64UntilDelimiterToOutput(
    ...
    output.writeAll(std::span<const Byte>(decoded), WRITE_COMPLETE_ERROR);
    return decoded.size();
}
...
[[nodiscard]] std::size_t copyDefaultCiphertextPayload(
    ...
    if (!has_profile_headers) {
        ...
        // kernel-to-kernel.
        sendfileExact(in_fd, out_fd, payload_start, embedded_file_size,
                      "Read Error: Failed while extracting encrypted payload.");
    ...
    while (cursor < embedded_file_size) {
        ...
        sendfileExact(in_fd, out_fd, payload_start + cursor, cap_by_header,
                      "Read Error: Failed while extracting encrypted payload.");
...
public:
    BlueskyCiphertextExtractor(
        ...
        : image_path_(image_path),
          ...
          output_(output_path, "Write Error: Unable to create output file. "
...
private:
    ...
    [[nodiscard]] std::uint16_t copyDataset(std::size_t dataset_size_index) {
        ...
        copyRangeFdToFd(input_fd_.fd(), output_.fd(), dataset_file_index, dataset_size);
...
[[nodiscard]] std::optional<std::size_t> findSignatureInStream(
    std::ifstream& input,
...
[[nodiscard]] std::size_t extractDefaultCiphertextToFile(
    ...
    FdOutputFile output(output_path, "Write Error: Unable to create output file. "
                                     "Make sure you have WRITE permissions for this location.");
