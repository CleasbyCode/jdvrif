...
[[nodiscard]] std::size_t copyDefaultCiphertextPayload(
    ...
    while (cursor < embedded_file_size) {
        ...
        const std::size_t cap_by_header = has_profile_headers
            ? std::min(embedded_file_size, next_header) - cursor
            : embedded_file_size - cursor;
...
public:
    BlueskyCiphertextExtractor(
        ...
        input_ = openBinaryInputOrThrow(image_path_, "Read Error: Failed to open image file.");
    ...
    [[nodiscard]] std::size_t run() {
        ...
        const std::uint16_t pshop_segment_size = readU16At(input_, pshop_segment_size_index);
...
private:
    ...
    [[nodiscard]] std::uint16_t copyDataset(std::size_t dataset_size_index) {
        ...
        const std::uint16_t dataset_size = readU16At(input_, dataset_size_index);
        if (dataset_size == 0 || dataset_size > remaining_) {
            throw std::runtime_error(CORRUPT_FILE_ERROR);
        }

        requireFileRange(image_size_, dataset_file_index, dataset_size, CORRUPT_FILE_ERROR);
        copyRangeToOutput(input_, output_, dataset_file_index, dataset_size);
...
[[nodiscard]] std::size_t extractDefaultCiphertextToFile(
    ...
    validateIccTrailingMarker(input, image_size, base_offset, total_profile_header_segments);
