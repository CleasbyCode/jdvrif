...
void findProfileHeaders(std::vector<uint_fast8_t>&Image_Vec, std::vector<uint_fast32_t>&Profile_Headers_Offset_Vec, std::string& file_name) {
	...
	file_name = { Image_Vec.begin() + ENCRYPTED_NAME_INDEX, Image_Vec.begin() + ENCRYPTED_NAME_INDEX + Image_Vec[ENCRYPTED_NAME_INDEX - 1] };

	for (int i = 0; i < 12; ++i) {
		file_name += Image_Vec[xor_key_index++]; 
    	}
