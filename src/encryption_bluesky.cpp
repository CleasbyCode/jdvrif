...
void reassembleBlueskyData(vBytes& jpg_vec, std::size_t sig_length) {
    ...
    if (pshop_segment_size > BLUESKY_SEGMENT_LAYOUT.dataset_max_size) {
        ...
        xmp_search_begin = checkedAdd(
            BLUESKY_CIPHER_LAYOUT.encrypted_payload_start_index,
            BLUESKY_SEGMENT_LAYOUT.exif_segment_data_size_limit,
            CORRUPT_FILE_ERROR);

        if (xmp_search_begin > pshop_segment_size_index || pshop_segment_size_index > jpg_vec.size()) {
            throw std::runtime_error(CORRUPT_FILE_ERROR);
        }
        auto xmp_opt = searchSig(
            std::span<const Byte>(jpg_vec).subspan(xmp_search_begin, pshop_segment_size_index - xmp_search_begin),
            BLUESKY_XMP_CREATOR_SIGNATURE);
        if (xmp_opt) {
            has_xmp_segment = true;
            xmp_creator_sig_index = xmp_search_begin + *xmp_opt;
