...
[[nodiscard]] std::span<const Byte> checkedDataRange(const vBytes& data, std::size_t offset, std::size_t size) {
    ...
    return std::span<const Byte>(data).subspan(offset, size);
}

void appendDataRange(vBytes& output, const vBytes& data, std::size_t offset, std::size_t size) {
    appendBytes(output, checkedDataRange(data, offset, size));
}
...
void appendXmpPayload(
    ...
    // Same checked slice the Photoshop datasets take (appendDataRange): the
    // arithmetic that produced this range is correct today, but this was the
    // one payload slice built by raw pointer arithmetic instead.
    const std::span<const Byte> remaining_data = checkedDataRange(
        data_vec,
        data_file_index,
        remaining_data_size);
