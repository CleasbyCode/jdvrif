...
*Mastodon (~6MB), Bluesky (~64KB / requries -b option), *Tumblr(~64KB), *X/Twitter (~10KB). *Limit measured by data file size only.
...
Bluesky cover image size limit (image + embedded secret file) = 1MB.
Secret file size limit: ~64KB. 

As the data file is compressed, you should be able to get significantly more than this 64KB limit, especially for text documents and other file types that compress well.
