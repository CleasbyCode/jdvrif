...
void displayInfo() {
 ...
 Bluesky (*see below / -b option), *Mastodon (~6MB), *Tumblr(~64KB), *X-Twitter (~10KB). 
 *Limit measured by data file size only.
 ...
 $ python3 bsky_post.py --handle exampleuser.bsky.social --password pxae-f17r-alp4-xqka --image jrif_11050.jpg
   --alt-text "text to describe image, here..." "standard text to appear in main post, here..."
 ...
 *Bluesky cover image size limit: 800KB.
 ...
 With X-Twitter, Bluesky, & Tumblr, the small size limits are measured by the compressed data file size and
 not the combined image + data file size. 

 As the embedded data file is compressed with jdvin using zlib/deflate, you should be able to get significantly
 more than the default size limit, especially for text documents and other file types that compress well. 

 You may wish to compress the data file yourself (zip, rar, 7z, etc) before embedding it with jdvin,
 so as to know exactly what the compressed data file size will be.

 Also with Mastodon, the size limit is measured by the compressed data file size and 
 not the combined image + data file size.
