...
void startJdv(std::string& image_file_name, std::string& data_file_name, bool isRedditOption) {
	...
	if (Image_Vec.size() > EXIF_POS) {
		const uint_fast16_t EXIF_BLOCK_SIZE = (static_cast<uint_fast16_t>(Image_Vec[EXIF_POS - 2]) << 8)
							| static_cast<uint_fast16_t>(Image_Vec[EXIF_POS - 1]);
	...
	Profile_Vec.reserve(data_file_size + Profile_Vec.size() + 65535);
	...
	insertProfileHeaders(Profile_Vec, File_Vec, data_file_size);
