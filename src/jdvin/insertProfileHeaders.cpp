...
void insertProfileHeaders(std::vector<uint_fast8_t>&Profile_Vec, std::vector<uint_fast8_t>&File_Vec) {
	...
	// Default profile and small data file fit within the first profile segment block.
	if (SEGMENT_SIZE + JPG_HEADER_LENGTH >= PROFILE_WITH_DATA_FILE_VEC_SIZE) {
		...
		if (last_segment_remainder_size > SEGMENT_SIZE * 2) {
		...
		} else if (last_segment_remainder_size > SEGMENT_SIZE) {
