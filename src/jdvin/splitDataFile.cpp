bool splitDataFile(std::vector<uint8_t>&segment_vec, std::vector<uint8_t>&data_file_vec) {
	constexpr uint8_t
		SEGMENT_HEADER_LENGTH = 18,
		JPG_HEADER_LENGTH     = 20,
		APP2_SIG_LENGTH       = 2; 

	const uint32_t COLOR_PROFILE_WITH_DATA_FILE_VEC_SIZE = static_cast<uint32_t>(segment_vec.size());
	...
	if (segment_data_size + JPG_HEADER_LENGTH + SEGMENT_HEADER_LENGTH >= COLOR_PROFILE_WITH_DATA_FILE_VEC_SIZE) { 
		...
		valueUpdater(segment_vec, SEGMENT_HEADER_SIZE_INDEX, SEGMENT_SIZE, value_bit_length);
		valueUpdater(segment_vec, COLOR_PROFILE_SIZE_INDEX, COLOR_PROFILE_SIZE, value_bit_length);
		segment_vec[SEGMENT_TOTAL_VALUE_INDEX] = 1; 
		...
		valueUpdater(segment_vec, SEGMENTS_TOTAL_VAL_INDEX, segments_required_approx_val, value_bit_length);
		...
		split_vec.reserve(segment_data_size + SEGMENT_REMAINDER_SIZE);
		...
		while (segments_required_approx_val--) {		
			if (!segments_required_approx_val && SEGMENT_REMAINDER_SIZE) {
				segment_data_size = SEGMENT_REMAINDER_SIZE;		
			   	valueUpdater(split_vec, segment_remainder_size_index, SEGMENT_REMAINDER_SIZE + SEGMENT_REMAINDER_DIFF, value_bit_length); 		
			}
			...
			if (segment_data_size == FIRST_SEGMENT_DATA_SIZE) {
			         data_file_vec.insert(data_file_vec.end(), split_vec.begin() + SEGMENT_HEADER_LENGTH, split_vec.end());
			} else {
				data_file_vec.insert(data_file_vec.end(), split_vec.begin(), split_vec.end());
    			}

    			split_vec.erase(split_vec.begin() + SEGMENT_HEADER_LENGTH, split_vec.end());
			segment_data_size = 65519;
			
			valueUpdater(split_vec, segments_sequence_value_index, ++segments_sequence_value, value_bit_length);
		}
