void splitDataFile(std::vector<uint8_t>&segment_vec, std::vector<uint8_t>&data_file_vec, bool& shouldDisplayMastodonWarning) {
	...
	if (icc_profile_with_data_file_vec_size > max_first_segment_size) { 
		...
		while (icc_segments_required--) {	
			if (!icc_segments_required) {
				if (icc_segment_remainder_size) {
					icc_segment_data_size = icc_segment_remainder_size;	
			   		valueUpdater(icc_segment_header_vec, icc_segment_remainder_size_index, (icc_segment_remainder_size + ICC_SEGMENT_HEADER_LENGTH), val_bit_length);
				} else {
					break;
				} 	
			}
			...
			valueUpdater(icc_segment_header_vec, icc_segments_sequence_val_index, ++icc_segments_sequence_val, val_bit_length);
		...
		valueUpdater(segment_vec, ICC_PROFILE_SIZE_INDEX, ICC_SEGMENT_SIZE, val_bit_length);
	...
	valueUpdater(data_file_vec, DEFLATED_DATA_FILE_SIZE_INDEX, static_cast<uint32_t>(data_file_vec.size()) - ICC_PROFILE_DATA_SIZE, val_bit_length);
}
