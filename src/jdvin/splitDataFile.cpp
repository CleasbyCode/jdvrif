void splitDataFile(std::vector<uint8_t>&segment_vec, std::vector<uint8_t>&data_file_vec, bool& shouldDisplayMastodonWarning) {
	...
	uint32_t color_profile_with_data_file_vec_size = static_cast<uint32_t>(segment_vec.size());
	...
	if (color_profile_with_data_file_vec_size > MAX_FIRST_SEGMENT_SIZE) { // Data file is too large for a single segment, so split data file in to multiple segments.
		...
		while (segments_required_val--) {	
			...
			if (segment_data_size != MAX_FIRST_SEGMENT_SIZE) {  
				 data_file_vec.insert(data_file_vec.end(), split_vec.begin(), split_vec.end()); 
			} else {	// Skip copying profile header as the first segment data already has one.
				data_file_vec.insert(data_file_vec.begin(), split_vec.begin() + PROFILE_HEADER_SEGMENT_LENGTH, split_vec.end()); 
    			}

    			split_vec.erase(split_vec.begin() + PROFILE_HEADER_SEGMENT_LENGTH, split_vec.end());
