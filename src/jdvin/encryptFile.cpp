...
uint64_t encryptFile(std::vector<uint8_t>& profile_vec, std::vector<uint8_t>& data_file_vec, std::string& data_filename, bool hasBlueskyOption) {
	...
	const uint16_t
		BLUESKY_DATA_FILE_INSERT_INDEX = 0x1D1,
	...
	if (hasBlueskyOption) {
		...
		valueUpdater(profile_vec, compressed_file_size_index, encrypted_vec.size(), value_bit_length);
		profile_vec.insert(profile_vec.begin() + BLUESKY_DATA_FILE_INSERT_INDEX, encrypted_vec.begin(), encrypted_vec.end());
	} else {
