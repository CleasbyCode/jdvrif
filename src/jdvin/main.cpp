...
int main(int argc, char** argv) {
	...
	if (argc == 2 && std::string(argv[1]) == "--info") {
		...
		const std::string IMAGE_FILE_NAME = isRedditOption ? argv[2] : argv[1];

		std::string 
			image_file_extension = IMAGE_FILE_NAME.length() > 3 ? IMAGE_FILE_NAME.substr(IMAGE_FILE_NAME.length() - 4) : IMAGE_FILE_NAME,
