...
grep -Eq '^Cover Image: [0-9]+KB, 1280x960, Progressive YCbCr 4:2:0, Source-derived Q65 quantization \(J-UNIWARD/STC\)\.$' "$WORK/capsize/capsize.log"
grep -Eq '^Theoretical J-UNIWARD/STC capacity limit for this cover image: +739 bytes \(~0KB\)\.$' "$WORK/capsize/capsize.log"
grep -Eq '^Conservative maximum compressed capacity with a 20-character filename: +625 bytes \(~0KB\)\.$' "$WORK/capsize/capsize.log"
grep -Eq '^Recommended  maximum compressed capacity with a 20-character filename: +0 bytes \(~0KB\)\.$' "$WORK/capsize/capsize.log"
grep -Fq 'consume 95 to 114 bytes' "$WORK/capsize/capsize.log"
grep -Fq 'at least' "$WORK/capsize/capsize.log"
grep -Fq '1KB below' "$WORK/capsize/capsize.log"
...
grep -Fq "Cover Image: ${CAPACITY_COVER_KB}KB, 1280x960, Progressive YCbCr 4:2:0, Source-derived Q65 quantization (J-UNIWARD/STC)." "$WORK/capacity/conceal.log"
grep -Eq '^Compressed data file \(payload\) size: [0-9]+ bytes \([0-9]+KB\)\.$' "$WORK/capacity/conceal.log"
grep -Eq '^Theoretical J-UNIWARD/STC capacity limit for this cover image: +739 bytes \(~0KB\)\.$' "$WORK/capacity/conceal.log"
grep -Eq '^Conservative maximum compressed capacity with a 20-character filename: +625 bytes \(~0KB\)\.$' "$WORK/capacity/conceal.log"
grep -Eq '^Recommended  maximum compressed capacity with a 20-character filename: +0 bytes \(~0KB\)\.$' "$WORK/capacity/conceal.log"
grep -Fq 'Data File Size Error:' "$WORK/capacity/conceal.log"
grep -Eq '^Already-compressed payload file size of [0-9]+ bytes \([0-9]+KB\) exceeds the recommended maximum limit of 0 bytes \(~0KB\) for this cover image\.$' "$WORK/capacity/conceal.log"
assert_no_output_image "$WORK/capacity"
PASS=$((PASS + 1))

# A second failure with a differently sized cover must report that source
# file's own decimal-KB size, rather than a cached or transcoded size.
...
ALT_COVER_KB="$((ALT_COVER_BYTES / 1000))"
if [[ "$ALT_COVER_KB" == "$CAPACITY_COVER_KB" ]]; then
    echo "X-Twitter cover-size fixtures do not exercise different displayed sizes" >&2
    exit 1
fi
grep -Fq "Cover Image: ${ALT_COVER_KB}KB, 1600x1200, Progressive YCbCr 4:2:0, Source-derived Q71 quantization (J-UNIWARD/STC)." "$WORK/capacity_alt_cover/conceal.log"
