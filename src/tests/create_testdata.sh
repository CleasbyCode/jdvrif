...
# Source for the synthetic two-quantization-table cover built by
# run_roundtrip_tests.sh; its DQT/DHT expectations assume these dimensions.
if [[ ! -f "$DATA/covers/cover_tables.jpg" ]]; then
    ffmpeg -hide_banner -loglevel error -y \
        -f lavfi -i testsrc=size=1600x1200:rate=1 -frames:v 1 -q:v 8 \
        "$DATA/covers/cover_tables.jpg"
fi
...
archive_path="$DATA/payloads/payload_archive.zip"
...
PY
    # ZIP stores source mtimes. Pin them and omit host-specific extra fields so
    # a fixture regenerated from scratch is byte-for-byte reproducible.
    chmod 600 "$archive_src/readme.txt" "$archive_src/chunk.dat"
    touch -t 202301010000 "$archive_src/readme.txt" "$archive_src/chunk.dat"
    (
        cd "$archive_src"
        zip -q -X ../payload_archive.zip readme.txt chunk.dat
    )
fi
