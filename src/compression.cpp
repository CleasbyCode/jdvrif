...
void deflateFromInputStream(std::istream& input, std::size_t expected_input_size, WriteChunkFn&& write_chunk) {
    ...
    z_stream strm{};
    if (deflateInit(&strm, selectCompressionLevel(expected_input_size)) != Z_OK) throw std::runtime_error("zlib: deflateInit failed");
...
void zlibCompressFileToPath(const fs::path& input_path, const fs::path& output_path, std::size_t expected_input_size) {
    ...
    deflateFromInputStream(input, expected_input_size, [&](std::span<const Byte> chunk) { writeBytesOrThrow(output, chunk, WRITE_COMPLETE_ERROR); });
