...
// libdeflate spans 1..12, but levels 10-12 are "ultra" tiers that cost vastly
// more time for marginal ratio gains. libdeflate level 9 gives ratio on par
// with (often slightly better than) zlib level 9 at a fraction of the time,
// which is the win we want. Do NOT map Z_BEST_COMPRESSION to 12.
[[nodiscard]] int libdeflateLevelFor(std::size_t input_size) {
    switch (selectCompressionLevel(input_size)) {
        ...
        case Z_BEST_COMPRESSION:    return 9;
        default:                    return 6;
    }
}
