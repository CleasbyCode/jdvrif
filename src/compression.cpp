...
[[nodiscard]] CompressionInputHints compressionInputHints(const fs::path& input_path) {
    ...
    if (!ec &&
        raw_input_size <= static_cast<std::uintmax_t>(std::numeric_limits<std::size_t>::max())) {
...
void reserveDeflateBoundIfAvailable(vBytes& output_vec, const CompressionInputHints& hints) {
    if (!hints.has_size ||
        hints.size_hint > static_cast<std::size_t>(std::numeric_limits<uLong>::max())) {
        return;
    }
    const uLong bound = compressBound(static_cast<uLong>(hints.size_hint));
    if (bound <= static_cast<uLong>(std::numeric_limits<std::size_t>::max())) {
        output_vec.reserve(static_cast<std::size_t>(bound));
    }
}
...
void deflateFromInputStream(std::istream& input, std::size_t input_size_hint, WriteChunkFn&& write_chunk) {
    ...
    z_stream strm{};
    if (deflateInit(&strm, selectCompressionLevel(input_size_hint)) != Z_OK) {
    ...
    while (!finished) {
        ...
        if (read_count < 0) {
            throw std::runtime_error("Read Error: Failed while compressing input file.");
        }
        if (!input && !input.eof()) {
            throw std::runtime_error("Read Error: Failed while reading input file.");
        }
...
[[nodiscard]] std::size_t inflateWithInputFeed(
    ...
    while (true) {
        ...
        const std::size_t produced = out_chunk.size() - strm.avail_out;
        if (produced > 0) {
            if (produced > output_limit || total_written > output_limit - produced) {
        ...
        if (ret == Z_BUF_ERROR) {
            ...
            if (input_left == 0 && strm.avail_in == 0) {
...
[[nodiscard]] vBytes deflateInMemory(std::span<const Byte> input) {
    vBytes buffer_vec(ZLIB_OUT_CHUNK_SIZE);
    vBytes output_vec;
    output_vec.reserve(input.size() + ZLIB_OUT_CHUNK_SIZE);

    z_stream strm{};
    strm.next_in   = const_cast<Byte*>(input.data());
    strm.next_out  = buffer_vec.data();
    strm.avail_out = static_cast<uInt>(ZLIB_OUT_CHUNK_SIZE);
    strm.avail_in  = 0;

    std::size_t input_left = input.size();

    auto flush_buffer = [&](std::size_t output_limit) {
        const std::size_t written = ZLIB_OUT_CHUNK_SIZE - strm.avail_out;
        if (written > 0) {
            if (written > output_limit || output_vec.size() > output_limit - written) {
                throw std::runtime_error("zlib inflate error: output exceeds safe size limit");
            }
            output_vec.insert(
                output_vec.end(),
                buffer_vec.begin(),
                buffer_vec.begin() + static_cast<std::ptrdiff_t>(written));
            strm.next_out  = buffer_vec.data();
            strm.avail_out = static_cast<uInt>(ZLIB_OUT_CHUNK_SIZE);
        }
    };
    ...
    if (deflateInit(&strm, selectCompressionLevel(input.size())) != Z_OK) {
        throw std::runtime_error("zlib: deflateInit failed");
    }
    struct DeflateGuard {
        z_stream* stream;
    ...
    do {
        ...
        if (strm.avail_out == 0) {
            flush_buffer(std::numeric_limits<std::size_t>::max());
        }
...
[[nodiscard]] vBytes inflateInMemory(std::span<const Byte> input, std::size_t output_limit) {
    vBytes buffer_vec(ZLIB_OUT_CHUNK_SIZE);
    vBytes output_vec;
    output_vec.reserve(input.size() + ZLIB_OUT_CHUNK_SIZE);
...
void zlibFunc(vBytes& data_vec, Mode mode) {
    if (mode == Mode::conceal) {
        data_vec = deflateInMemory(std::span<const Byte>(data_vec));
        return;
    }
    data_vec = inflateInMemory(std::span<const Byte>(data_vec), ZLIB_MAX_INFLATED);
}
...
void zlibCompressFile(const fs::path& input_path, vBytes& output_vec) {
    ...
    reserveDeflateBoundIfAvailable(output_vec, hints);

    deflateFromInputStream(input, hints.size_hint, [&](std::span<const Byte> chunk) {
        output_vec.insert(output_vec.end(), chunk.begin(), chunk.end());
    });
}
...
void zlibInflateFileToFile(const fs::path& input_path, const fs::path& output_path) {
    std::ifstream input = openBinaryInputOrThrow(
        input_path,
        std::format("Failed to open file for inflate: {}", input_path.string()));

    const std::size_t input_size = checkedFileSize(
        input_path,
    ...
    const std::size_t total_written = inflateWithInputFeed(
        ...
        [&](z_stream& strm, std::size_t& left) {
            if (strm.avail_in == 0 && left > 0) {
                const std::size_t chunk = std::min(left, in_chunk.size());
                readExactOrThrow(
                    input,
                    in_chunk.data(),
                    chunk,
                    "Read Error: Failed while reading compressed input file.");
                strm.next_in = in_chunk.data();
                strm.avail_in = static_cast<uInt>(chunk);
                left -= chunk;
            }
