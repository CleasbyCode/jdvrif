...
[[nodiscard]] std::size_t computeStreamEncryptedSizePrefixed(
    ...
    const std::size_t total_plaintext_size = input_plaintext_size + prefix_plaintext_size;
    if (total_plaintext_size == 0) {
        return 0;
    }

    const auto count_chunks = [](std::size_t n) -> std::size_t {
        if (n == 0) {
            return 0;
        }
        return n / STREAM_CHUNK_SIZE + ((n % STREAM_CHUNK_SIZE) != 0 ? 1 : 0);
    };

    const std::size_t prefix_chunks = count_chunks(prefix_plaintext_size);
    const std::size_t input_chunks = count_chunks(input_plaintext_size);
    if (prefix_chunks > std::numeric_limits<std::size_t>::max() - input_chunks) {
        throw std::runtime_error("File Size Error: Encrypted output overflow.");
    }
    const std::size_t chunk_count = prefix_chunks + input_chunks;

    const std::size_t per_chunk_overhead = STREAM_ABYTES + STREAM_FRAME_LEN_BYTES;
    if (chunk_count > (std::numeric_limits<std::size_t>::max() - total_plaintext_size) / per_chunk_overhead) {
        throw std::runtime_error("File Size Error: Encrypted output overflow.");
    }
    return total_plaintext_size + chunk_count * per_chunk_overhead;
}
...
void encryptFileWithSecretStreamPrefixed(
    ...
    while (prefix_off < prefix_plaintext.size()) {
        const std::size_t mlen = std::min(STREAM_CHUNK_SIZE, prefix_plaintext.size() - prefix_off);
        const bool is_final = (remaining_plain == mlen);
        pushPlainChunk(prefix_plaintext.subspan(prefix_off, mlen), is_final);
        prefix_off += mlen;
    ...
    while (input_left > 0) {
        const std::size_t mlen = std::min(STREAM_CHUNK_SIZE, input_left);
        const std::streamsize read_count = readSomeOrThrow(
            input,
            in_chunk.data(),
            mlen,
            "Read Error: Failed while reading input file.");
        if (read_count != static_cast<std::streamsize>(mlen)) {
            throw std::runtime_error("Read Error: Failed to read full input while encrypting.");
        }
        const bool is_final = (remaining_plain == mlen);
        pushPlainChunk(std::span<const Byte>(in_chunk.data(), mlen), is_final);
        sodium_memzero(in_chunk.data(), mlen);
