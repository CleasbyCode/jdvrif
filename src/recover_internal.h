...
[[nodiscard]] std::optional<std::size_t> findSignatureInStream(std::ifstream& input, std::span<const Byte> sig, std::size_t search_limit = 0, std::size_t start_offset = 0);
[[nodiscard]] std::optional<std::size_t> findSignatureInFile(const fs::path& path, std::span<const Byte> sig, std::size_t search_limit = 0, std::size_t start_offset = 0);
