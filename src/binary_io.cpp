...
namespace {
template <typename T>
[[nodiscard]] T toBigEndian(T value) {
...
template <typename T>
[[nodiscard]] T fromBigEndian(T value) {
    if constexpr (std::endian::native == std::endian::little) {
        return std::byteswap(value);
    } else {
        return value;
...
std::optional<std::size_t> searchSig(std::span<const Byte> v, std::span<const Byte> sig, std::size_t limit) {
    if (sig.empty() || v.empty() || sig.size() > v.size()) {
        return std::nullopt;
    }
    if (limit != 0 && limit < sig.size()) {
        return std::nullopt;
    }

    const auto search_span = (limit == 0 || limit >= v.size())
        ? v
        : v.first(limit);
...
void updateValue(std::span<Byte> data, std::size_t index, std::size_t value, std::size_t length) {
    ...
    auto write = [&]<typename T>() {
        T narrowed = checkedNarrow<T>(value, length);
        const T encoded = toBigEndian(narrowed);
...
std::size_t getValue(std::span<const Byte> data, std::size_t index, std::size_t length) {
    ...
    auto read = [&]<typename T>() -> std::size_t {
        ...
        return static_cast<std::size_t>(fromBigEndian(encoded));
    };
