...
void recoverData(const fs::path& image_file_path) {
    ...
    // once it starts -- MAX_IMAGE_PIXELS caps the coefficient count,
    // carrierCapacity() caps the declared payload against it, and
    // REDDIT_UPLOAD_SIZE_LIMIT caps the read below. Keep those three caps in
    // mind before raising any of them.
    if (image_file_size <= REDDIT_UPLOAD_SIZE_LIMIT) {
        ...
        if (auto envelope = extractRedditEnvelope(image, deriveCarrierKeyFromPin(recovery_pin))) {
