...
void decryptAndFinalizeFromCipherStage(
    ...
    DecryptResult decrypt_result = decryptDataFile(
        ...
        is_bluesky_file,
        {
            .stream_output_path = &stream_stage.path,
            .encrypted_input_path = &cipher_stage.path,
    ...
    if (!decrypt_result.used_stream_output) {
        throw std::runtime_error("Internal Error: Recovery did not produce a staged output file.");
    }
...
[[nodiscard]] std::optional<std::size_t> findEmbeddedIccProfile(const fs::path& image_file_path, std::size_t image_file_size) {
    ...
    while (search_start < image_file_size) {
        const auto icc_opt = findSignatureInStream(input, ICC_PROFILE_SIGNATURE, 0, search_start);
