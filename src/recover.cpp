...
[[nodiscard]] fs::path safeRecoveryPath(std::string decrypted_filename) {
    ...
    if (ec) throw std::runtime_error(std::format("Write Error: Failed to inspect recovery output path: {}", ec.message()));
    if (!candidate_exists) return candidate;
    ...
    for (std::size_t i = 1; i <= 10000; ++i) {
        ...
        if (ec_to_chars != std::errc{}) throw std::runtime_error("Write Error: Unable to create a unique output filename.");
...
void commitRecoveredOutput(TempFileCleanupGuard& staged_file, const fs::path& output_path) {
    commitStagedFileNoReplaceOrThrow(staged_file.path, output_path, "Write Error: Failed to commit recovered file");
    staged_file.dismiss();
}
...
void decryptAndFinalizeFromCipherStage(vBytes& metadata_vec, bool is_bluesky_file, bool is_data_compressed, TempFileCleanupGuard& cipher_stage, TempFileCleanupGuard& stream_stage) {
    ...
    const fs::path output_path = safeRecoveryPath(std::move(decrypt_result.filename));
    commitRecoveredOutput(stream_stage, output_path);
...
void recoverData(const fs::path& image_file_path) {
    ...
    if (!jdvrif_sig_opt) throw std::runtime_error("Image File Error: Signature check failure. This is not a valid jdvrif \"file-embedded\" image.");
    ...
    if (icc_opt) { recoverFromIccPath(image_file_path, image_file_size, *icc_opt); return; }
    recoverFromBlueskyPath(image_file_path, image_file_size, jdvrif_sig_index, JDVRIF_SIGNATURE);
}
