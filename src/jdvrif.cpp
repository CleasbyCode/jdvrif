...
void openFiles(char* argv[], jdvStruct& jdv) {
	...
	if (jdv.MODE == "-x") { // Extract mode. 
		...
		// Shorter signature index location within vector "EmbdImageVec", this is caused by Imgur, which strips part of the JPG header.
		...
		// If JPG header has been shortened (Imgur), we need to put back the standard header.
		...
		// 200MB file size limit for this program. 
...
void encryptDecrypt(jdvStruct& jdv) {
	...
	while (FILE_SIZE > indexPos) {
		...
		if (indexPos >= NAME_LENGTH) {
			// Reset filename character position to the start if it's reached last character.
...
void insertProfileBlocks(jdvStruct& jdv) {
	...
	SBYTE
		...
		profileSecondaryBlockSizeIndex = 40,
	...
	// If we embed multiple data files (max 6), each outputted image will be differentiated 
	// by a number in the name, jdv_img1.jpg, jdv_img2.jpg, jdv_img3.jpg, etc.
