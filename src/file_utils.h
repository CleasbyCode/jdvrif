...
inline void requireSpanRange(std::span<const Byte> data, std::size_t index, std::size_t length, const char* error_message) {
    if (!spanHasRange(data, index, length)) throw std::runtime_error(error_message);
}

[[nodiscard]] inline std::size_t checkedAdd(std::size_t a, std::size_t b, const char* error_message) {
    if (a > std::numeric_limits<std::size_t>::max() - b) throw std::overflow_error(error_message);
    return a + b;
}

[[nodiscard]] inline std::size_t checkedMul(std::size_t a, std::size_t b, const char* error_message) {
    if (a != 0 && b > std::numeric_limits<std::size_t>::max() / a) throw std::overflow_error(error_message);
    return a * b;
}

[[nodiscard]] inline std::streamoff checkedStreamOffset(std::size_t offset, const char* error_message) {
    if (offset > static_cast<std::size_t>(std::numeric_limits<std::streamoff>::max())) throw std::overflow_error(error_message);
...
[[nodiscard]] std::optional<fs::path> makeUniqueRandomizedPath(const fs::path& parent_dir, std::string_view prefix, std::string_view suffix, std::size_t max_attempts = 1024, std::size_t token_hex_chars = 16);
[[nodiscard]] fs::path uniqueRandomizedPathOrThrow(const fs::path& parent_dir, std::string_view prefix, std::string_view suffix, std::size_t max_attempts, std::string_view error_message, std::size_t token_hex_chars = 16);
...
struct TempFileCleanupGuard {
    fs::path path{}; bool active{false};

    TempFileCleanupGuard() = default;
    explicit TempFileCleanupGuard(fs::path p) { set(std::move(p)); }
    TempFileCleanupGuard(const TempFileCleanupGuard&) = delete; TempFileCleanupGuard& operator=(const TempFileCleanupGuard&) = delete;

    TempFileCleanupGuard(TempFileCleanupGuard&& other) noexcept : path(std::move(other.path)), active(other.active) { other.dismiss(); }

    TempFileCleanupGuard& operator=(TempFileCleanupGuard&& other) noexcept {
        if (this != &other) {
            if (active) cleanupPathNoThrow(path);
    ...
    void set(fs::path p) { if (active) cleanupPathNoThrow(path); path = std::move(p); active = !path.empty(); }
    void dismiss() noexcept { path.clear(); active = false; }
    ~TempFileCleanupGuard() { if (active) cleanupPathNoThrow(path); }
};
