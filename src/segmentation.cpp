...
constexpr const char* SIZE_OVERFLOW_ERROR = "File Size Error: Segment output size overflow.";
...
struct PayloadSource {
    ...
    PayloadSource(
        ...
        throwIf(encrypted_fd < 0, "Read Error: Failed to open encrypted payload.");
    ...
    void copyTo(OutputFile& output, std::size_t size) {
        ...
        if (left > 0) {
            throwIf(left > encrypted_left, "Read Error: Encrypted payload is shorter than expected.");
...
void copyFileToOutput(const fs::path& input_path, OutputFile& output, std::size_t length) {
    ...
    throwIf(!payload_source.exhausted(), "Read Error: Encrypted payload is longer than expected.");
}

[[nodiscard]] std::size_t requiredIccSegments(std::size_t payload_size) {
    throwIf(payload_size == 0, "File Size Error: Segment count exceeds supported limit.");
    ...
    throwIf(
        ...
        "File Size Error: Segment count exceeds supported limit.");
...
[[nodiscard]] SegmentedEmbedSummary writeIccDataToOutput(
    ...
    throwIf(segment_vec.size() < INITIAL_HEADER_BYTES, "File Extraction Error: Corrupt segment header.");

    const std::size_t single_segment_size = checkedAdd(segment_vec.size(), encrypted_size, SIZE_OVERFLOW_ERROR);
    ...
    if (single_segment_size <= MAX_SINGLE_SEGMENT_SIZE) {
        ...
        writeOutput(output, std::span<const Byte>(segment_vec));
    ...
    const std::size_t payload_size = checkedAdd(payload_prefix_size, encrypted_size, SIZE_OVERFLOW_ERROR);
    ...
    const std::size_t icc_data_size = checkedAdd(
        SOI_SIG_LENGTH,
        checkedAdd(payload_size, total_header_bytes, SIZE_OVERFLOW_ERROR),
        SIZE_OVERFLOW_ERROR);
    ...
    throwIf(payload_left != 0 || !payload_source.exhausted(), "Read Error: Encrypted payload size mismatch.");
...
[[nodiscard]] SegmentedEmbedSummary writeEmbeddedJpgFromEncryptedFile(
    ...
    SegmentedEmbedSummary summary = writeIccDataToOutput(output, segment_vec, encrypted_path, encrypted_size);
