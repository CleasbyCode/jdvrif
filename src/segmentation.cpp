...
[[nodiscard]] std::size_t checkedAdd(std::size_t a, std::size_t b, const char* error_message) {
...
[[nodiscard]] std::size_t checkedMul(std::size_t a, std::size_t b, const char* error_message) {
...
struct PayloadSource {
    ...
    void readExact(Byte* dst, std::size_t size) {
        ...
        if (template_offset < template_payload.size()) {
            ...
            std::memcpy(
                dst,
                template_payload.data() + static_cast<std::ptrdiff_t>(template_offset),
                take);
...
void copyPayloadToOutput(PayloadSource& payload_source, std::ostream& output, std::size_t length) {
    ...
    while (left > 0) {
        ...
        writeBytesOrThrow(output, std::span<const Byte>(buffer.data(), chunk), "Write Error: Failed to write complete output file.");
        left -= chunk;
    }
}
...
void writeRedditPaddingToOutput(std::ostream& output) {
    ...
    writeBytesOrThrow(output, PADDING_PREFIX, "Write Error: Failed to write complete output file.");
    ...
    while (remaining > 0) {
        ...
        writeBytesOrThrow(output, std::span<const Byte>(padding_chunk.data(), chunk), "Write Error: Failed to write complete output file.");
        remaining -= chunk;
    }
}
...
[[nodiscard]] SegmentedEmbedSummary writeIccDataToOutput(
    ...
    const std::size_t max_first_segment_size =
    ...
    if (single_segment_size <= max_first_segment_size) {
        ...
        writeBytesOrThrow(output, leading, "Write Error: Failed to write complete output file.");
    ...
    if (!omit_soi) {
        writeBytesOrThrow(
            output,
            std::span<const Byte>(segment_vec.data(), SOI_SIG_LENGTH),
    ...
    for (std::size_t seg = 1; seg <= segments_required; ++seg) {
        ...
        const auto header = makeIccHeader(
            static_cast<uint16_t>(data_size + SEGMENT_HEADER_LENGTH),
            seg);
        writeBytesOrThrow(output, header, "Write Error: Failed to write complete output file.");
...
[[nodiscard]] SegmentedEmbedSummary writeEmbeddedJpgFromEncryptedFile(
    ...
    if (!has_reddit_option) {
        SegmentedEmbedSummary summary = writeIccDataToOutput(
            output,
            segment_vec,
            encrypted_path,
            encrypted_size,
            false);
        writeBytesOrThrow(output, jpg_vec, "Write Error: Failed to write complete output file.");
    ...
    writeBytesOrThrow(
        output,
        jpg_vec.first(jpg_vec.size() - EOI_SIG_LENGTH),
        "Write Error: Failed to write complete output file.");
