...
struct PayloadSource {
    ...
    PayloadSource(
        ...
        : template_payload(template_payload_arg),
          encrypted_input(openBinaryInputOrThrow(encrypted_path, "Read Error: Failed to open encrypted payload.")),
...
void writeOutput(std::ostream& output, std::span<const Byte> bytes) {
    writeBytesOrThrow(output, bytes, WRITE_COMPLETE_ERROR);
}
...
[[nodiscard]] SegmentedEmbedSummary writeIccDataToOutput(
    ...
    // The scratch buffer only needs to hold the data portion of one segment;
    // the 18-byte header is streamed directly to the output. The 1 MiB
    // pubsetbuf on the ofstream (see conceal.cpp::writeToStagedOutput)
    // coalesces the two writes per segment into a single write(2) syscall.
    thread_local vBytes payload_chunk(SEGMENT_DATA_SIZE);
    ...
    for (std::size_t seg = 1; seg <= segments_required; ++seg) {
        ...
        payload_source.readExact(payload_chunk.data(), data_size);
        writeOutput(output, std::span<const Byte>(payload_chunk.data(), data_size));
