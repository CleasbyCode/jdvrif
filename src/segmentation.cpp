...
[[nodiscard]] SegmentedEmbedSummary writeIccDataToOutput(
    ...
    thread_local vBytes segment_buf(SEGMENT_HEADER_LENGTH + SEGMENT_DATA_SIZE);
    ...
    for (std::size_t seg = 1; seg <= segments_required; ++seg) {
        ...
        std::memcpy(segment_buf.data(), header.data(), header.size());
        payload_source.readExact(segment_buf.data() + header.size(), data_size);
        writeOutput(output, std::span<const Byte>(segment_buf.data(), header.size() + data_size));
