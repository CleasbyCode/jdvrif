...
void validateBlueskyExifCapacity(
    ...
    // Full JPEG APP1/EXIF segment size ceiling (0xFFFF minus the 2-byte length
    // field itself). Distinct from BLUESKY_SEGMENT_LAYOUT.exif_segment_data_size_limit
    // (65027), which is the smaller cap on how many *payload* bytes conceal packs
    // into the EXIF segment; here we only sanity-check the raw segment size field.
