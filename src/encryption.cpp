...
std::size_t encryptDataFileFromPathToFile(
    vBytes& segment_vec,
    const fs::path& data_path,
    std::size_t input_size,
    const std::string& data_filename,
    bool hasBlueskyOption,
...
std::string decryptDataFile(vBytes& jpg_vec, bool isBlueskyFile, bool& hasDecryptionFailed,
    ...
    auto parse_payload_info = [&]() -> PayloadInfo {
        ...
        if (!isBlueskyFile) {
            requireSpanRange(
                jpg_vec,
                ICC_SEGMENT_LAYOUT.embedded_total_profile_header_segments_index,
                2,
                CORRUPT_FILE_ERROR);
            total_profile_header_segments = static_cast<uint16_t>(
                getValue(jpg_vec, ICC_SEGMENT_LAYOUT.embedded_total_profile_header_segments_index));
        }
        ...
        if (total_profile_header_segments && !isBlueskyFile && !has_external_encrypted_input) {
            ...
            if (!marker_index ||
                !spanHasRange(jpg_vec, *marker_index, JPEG_APP2_MARKER.size()) ||
                !std::equal(
                    JPEG_APP2_MARKER.begin(),
                    JPEG_APP2_MARKER.end(),
                    jpg_vec.begin() + static_cast<std::ptrdiff_t>(*marker_index))) {
    ...
    auto prepare_encrypted_payload = [&](const PayloadInfo& payload) -> bool {
        ...
        if (!isBlueskyFile && payload.total_profile_header_segments) {
            ...
            std::size_t
                read_pos    = 0,
                write_pos   = 0,
                next_header = ICC_SEGMENT_LAYOUT.profile_header_insert_index;
