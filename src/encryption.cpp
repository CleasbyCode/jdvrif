...
std::uint64_t encryptDataFileCommon(vBytes& segment_vec, vBytes& jpg_vec, vString& platforms_vec,
    ...
    if (hasBlueskyOption) buildBlueskySegments(segment_vec, encrypted_vec);
    else segment_vec.insert(segment_vec.end(), encrypted_vec.begin(), encrypted_vec.end());
    encrypted_vec.clear();
...
std::uint64_t encryptDataFileFromPath(vBytes& segment_vec, const fs::path& data_path, std::size_t input_size,
    ...
    return encryptDataFileCommon(
        ...
        hasBlueskyOption,
        hasRedditOption,
        [&](const Key& key, std::array<Byte, crypto_secretstream_xchacha20poly1305_HEADERBYTES>& stream_header, vBytes& encrypted_vec, std::span<const Byte> filename_prefix) {
            encryptFileWithSecretStreamPrefixed(data_path, input_size, filename_prefix, key, stream_header, encrypted_vec);
        });
}
