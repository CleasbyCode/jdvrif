...
DecryptResult decryptDataFile(vBytes& jpg_vec, bool isBlueskyFile, const DecryptRequest& request) {
    ...
    auto prepare_encrypted_payload = [&](const PayloadInfo& payload) -> bool {
        ...
        if (!isBlueskyFile && payload.total_profile_header_segments) {
            // Single pass: shift payload to front while skipping strided ICC profile headers.
            ...
            while (read_rel < total) {
                if (read_rel == next_header) {
                    ...
                    next_header = checkedAdd(next_header, ICC_SEGMENT_LAYOUT.per_segment_stride, CORRUPT_FILE_ERROR);
                    continue;
                }
                const std::size_t run_size = std::min(total, next_header) - read_rel;
                std::memmove(jpg_vec.data() + write_pos, jpg_vec.data() + src_base + read_rel, run_size);
                read_rel += run_size;
                write_pos += run_size;
            }
            jpg_vec.resize(write_pos);
        } else {
            std::memmove(jpg_vec.data(), jpg_vec.data() + src_base, total);
            jpg_vec.resize(total);
        }
