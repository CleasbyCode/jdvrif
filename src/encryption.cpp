...
std::uint64_t encryptDataFileForBluesky(
    ...
    // Caller writes segment_vec and the cover JPEG bytes as two consecutive
    // spans into the output stream, so pre-concatenating into jpg_vec is a
    // pure pessimization (a full-cover memmove on every Bluesky conceal).
    (void)jpg_vec;
