...
[[nodiscard]] std::optional<std::span<const Byte>> exifPayload(std::span<const Byte> jpg) {
    ...
    while (pos + 4 <= jpg.size() && pos < scan_limit) {
        ...
        if (!spanHasRange(jpg, pos, 2)) return std::nullopt;
        const uint16_t segment_length = static_cast<uint16_t>((static_cast<uint16_t>(jpg[pos]) << 8) | static_cast<uint16_t>(jpg[pos + 1]));
        if (segment_length < 2 || !spanHasRange(jpg, pos, segment_length)) return std::nullopt;
