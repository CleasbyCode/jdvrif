...
namespace {
constexpr std::size_t EXIF_SEARCH_LIMIT = 4096, EXIF_HEADER_SIZE = 6, TIFF_HEADER_SIZE = 8, IFD_ENTRY_SIZE = 12, DQT_SEARCH_LIMIT = 32768,
                      DQT_8BIT_TABLE_SIZE = 64, DQT_16BIT_TABLE_SIZE = 128, DEFAULT_QUALITY_ESTIMATE = 80;
...
[[nodiscard]] std::optional<std::span<const Byte>> exifPayload(std::span<const Byte> jpg) {
    ...
    if (!spanHasRange(jpg, pos, 4)) return std::nullopt;

    const uint16_t segment_length = static_cast<uint16_t>((static_cast<uint16_t>(jpg[pos + 2]) << 8) | static_cast<uint16_t>(jpg[pos + 3]));

    if (segment_length < 2 || !spanHasRange(jpg, pos + 2, segment_length)) {
        return std::nullopt;
    }

    const std::size_t payload_offset = pos + 4;
    const std::size_t payload_size = segment_length - 2;
...
[[nodiscard]] int qualityFromLuminanceSum(int sum) {
    ...
    for (std::size_t q = 1; q < STD_LUMINANCE_SUMS.size(); ++q) {
        if (sum >= STD_LUMINANCE_SUMS[q]) {
            if (q > 1) {
                const int diff_current = sum - STD_LUMINANCE_SUMS[q], diff_prev = STD_LUMINANCE_SUMS[q - 1] - sum;
...
std::optional<uint16_t> exifOrientation(std::span<const Byte> jpg) {
    ...
    if (payload.size() < EXIF_HEADER_SIZE || !std::ranges::equal(payload.first(EXIF_HEADER_SIZE), EXIF_SIG)) return std::nullopt;
...
int estimateImageQuality(std::span<const Byte> jpg) {
    ...
    const std::size_t length = (static_cast<std::size_t>(jpg[pos + 2]) << 8) | jpg[pos + 3], end = pos + 2 + length;
    ...
    while (pos < end) {
        const Byte header = jpg[pos++], precision = (header >> 4) & 0x0F, table_id = header & 0x0F;
...
void optimizeImage(vBytes& jpg_vec, bool isProgressive) {
    if (jpg_vec.empty()) throw std::runtime_error("JPG image is empty!");

    auto transformer = TJHandle::makeTransformer();
    if (!transformer) throw std::runtime_error("tjInitTransform() failed");
